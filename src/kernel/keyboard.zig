...
pub const Keyboard = struct {
    ...
    pub fn isFull(self: *const Keyboard) bool {
        ...
        end_plus_one = @addWithOverflow(self.queue_end, 1)[0];
    ...
    pub fn writeKey(self: *Keyboard, key: KeyAction) bool {
        if (!self.isFull()) {
            ...
            self.queue_end = @addWithOverflow(self.queue_end, 1)[0];
            return true;
        }
        return false;
    }
    ...
    pub fn readKey(self: *Keyboard) ?KeyAction {
        ...
        self.queue_front = @addWithOverflow(self.queue_front, 1)[0];
        return key;
    }
    ...
    test "writeKey" {
        ...
        inline while (i < QUEUE_SIZE - 1) : (i += 1) {
            try testing.expectEqual(keyboard.writeKey(.{
                .position = @as(KeyPosition, @enumFromInt(i)),
                .released = false,
            }), true);
            try testing.expectEqual(keyboard.queue[i].position, @as(KeyPosition, @enumFromInt(i)));
            try testing.expectEqual(keyboard.queue_end, i + 1);
            try testing.expectEqual(keyboard.queue_front, 0);
        }

        try testing.expectEqual(keyboard.writeKey(.{
            .position = @as(KeyPosition, @enumFromInt(33)),
            .released = false,
        }), false);
        try testing.expect(keyboard.isFull());
    }
    ...
    test "readKey" {
        ...
        i = 0;
        inline while (i < QUEUE_SIZE - 1) : (i += 1) {
            try testing.expectEqual(keyboard.readKey().?.position, @as(KeyPosition, @enumFromInt(i)));
