...
fn handleRead(ctx: *const arch.CpuState, node_handle: usize, buff_ptr: usize, buff_len: usize, ignored1: usize, ignored2: usize) anyerror!usize {
    ...
    const real_handle = @as(task.Handle, @intCast(node_handle));
...
test "handleOpen" {
    ...
    var test_handle = @as(task.Handle, @intCast(try handleOpen(&empty, @intFromPtr(name1.ptr), name1.len, @intFromEnum(vfs.OpenFlags.CREATE_FILE), 0, undefined)));
    ...
    test_handle = @as(task.Handle, @intCast(try handleOpen(&empty, @intFromPtr(name2.ptr), name2.len, @intFromEnum(vfs.OpenFlags.CREATE_DIR), 0, undefined)));
    ...
    test_handle = @as(task.Handle, @intCast(try handleOpen(&empty, @intFromPtr(name3.ptr), name3.len, @intFromEnum(vfs.OpenFlags.CREATE_FILE), 0, undefined)));
    test_node = (try current_task.getVFSHandle(test_handle)).?;
    ...
    test_handle = @as(task.Handle, @intCast(try handleOpen(&empty, @intFromPtr(name3.ptr), name3.len, @intFromEnum(vfs.OpenFlags.NO_CREATION), 0, undefined)));
    test_node = (try current_task.getVFSHandle(test_handle)).?;
    try testing.expectEqual(testfs.tree.children.items[1].children.items.len, 1);
...
test "handleRead" {
    ...
    const test_file = @as(task.Handle, @intCast(try handleOpen(&empty, @intFromPtr(test_file_path.ptr), test_file_path.len, @intFromEnum(vfs.OpenFlags.CREATE_FILE), 0, undefined)));
    ...
    const test_link = @as(task.Handle, @intCast(try handleOpen(&empty, @intFromPtr(link.ptr), link.len, @intFromEnum(vfs.OpenFlags.CREATE_SYMLINK), @intFromPtr(args), undefined)));
    {
