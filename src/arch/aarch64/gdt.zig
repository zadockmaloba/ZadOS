...
test "GDT entries" {
    ...
    try expectEqual(@as(u64, 0xCF9A000000FFFF), @as(u16, @bitCast(kernel_code_entry)));

    const kernel_data_entry = gdt_entries[KERNEL_DATA_INDEX];
    try expectEqual(@as(u64, 0xCF92000000FFFF), @as(u16, @bitCast(kernel_data_entry)));

    const user_code_entry = gdt_entries[USER_CODE_INDEX];
    try expectEqual(@as(u64, 0xCFFA000000FFFF), @as(u16, @bitCast(user_code_entry)));

    const user_data_entry = gdt_entries[USER_DATA_INDEX];
    try expectEqual(@as(u64, 0xCFF2000000FFFF), @as(u16, @bitCast(user_data_entry)));
...
test "init" {
    ...
    var expected: u64 = 0;
    expected |= @as(u64, @as(u16, @truncate(tss_limit)));
    expected |= @as(u64, @as(u24, @truncate(tss_addr))) << 16;
    expected |= @as(u64, 0x89) << (16 + 24);
    expected |= @as(u64, @as(u4, @truncate(tss_limit >> 16))) << (16 + 24 + 8);
    // Flags are zero
    expected |= @as(u64, @as(u8, @truncate(tss_addr >> 24))) << (16 + 24 + 8 + 4 + 4);

    try expectEqual(expected, @as(u64, @bitCast(tss_entry)));
